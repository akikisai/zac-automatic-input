...
このツールの通信は以下の 2 段構成です。

```
[ブラウザ] --HTTP--> [localhost:5000 Flask] --HTTPS--> [ZAC API]
```

| 区間 | プロトコル | 流れるもの |
|------|----------|-----------|
| ブラウザ → localhost | HTTP | Flask のセッション ID・CSRF トークンのみ |
| Flask → ZAC API | HTTPS（TLS 暗号化） | ZAC セッション Cookie・API リクエスト |

**ZAC の Cookie やパスワードはブラウザに渡しません。**
 サーバーメモリのみで保持し、ZAC API への通信は暗号化された HTTPS で行うため、盗聴リスクは ZAC をブラウザで直接操作する場合と同等です。

ZAC 側から見ると「有効な Cookie を持ったクライアントが REST API を叩いている」として認識され、ログインは正規の画面（Selenium 経由）で行います。

## セキュリティ上の前提

このツールは**各自のローカル PC でのみ動かす前提**で設計されています。

### 実装済みの対策

- 公開ポートは `127.0.0.1:5000` のみ（外部から到達不可）
- ZAC Cookie はサーバーメモリのみ保持（ブラウザに非露出）
- Flask → ZAC API は HTTPS 通信（証明書検証あり）
- セッションはユーザー単位で分離
- POST API は CSRF トークンで保護
- セッション Cookie は HttpOnly / SameSite=Strict
- ブラウザキャッシュ抑止ヘッダーを全レスポンスに付与

### 利用時に守ること

- **外部公開しない**（サーバーとして他人に使わせない）
...
### ZAC 側の仕様変更による使用不能リスクについて

このツールが現時点で動作しているのは、**ZAC がブラウザ以外からの API アクセスを制限していない**ためです。
ZAC 側がリファラーチェック・IP 制限・クライアント証明書・追加認証などを導入した場合、このツールが使用できなくなる可能性があります。
ZAC のアップデートや社内ポリシーの変更後に動作しなくなった場合は、この点を最初に疑ってください。